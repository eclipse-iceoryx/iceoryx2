...
Test functions are annotated with `#[inventory_test]` to register them to be
run by both the `std` and `no_std` test runners:
...
(via `catch_unwind`) but ignored by the `no_std` runner. The `#[required_std]`
should be used in conjunction to skip panic tests in `no_std` contexts.
