...
#define DURATION_0S 0
#define DURATION_500MS 500000000
...
static iox2_degradation_action_e degradation_handler(iox2_degradation_cause_e degradation_cause,
                                                     iox2_degradation_context_h_ref degradation_context,
                                                     iox2_callback_context ctx) {
    if (degradation_cause == iox2_degradation_cause_e_UNABLE_TO_DELIVER_DATA) {
        printf("Discarded sample from service id: %lu\n", iox2_degradation_context_service_id(degradation_context));
        if (ctx) {
...
int main(void) {
    ...
    ctx.fail_counter = 0;
    iox2_port_factory_publisher_builder_set_degradation_callback(&publisher_builder, degradation_handler, &ctx);
