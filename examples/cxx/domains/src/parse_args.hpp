...
struct CliOption {
    ...
    iox2::legacy::string<N> default_value;
    std::string error_string;
};

template <uint64_t N>
auto parse_from_args(int argc, char** argv, CliOption<N> opt) -> iox2::legacy::string<N> {
    for (int i = 0; i < argc; ++i) {
        ...
        if (arg == opt.short_option || arg == opt.long_option) {
            if (i + 1 < argc) {
                // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic) required to parse args
                return { iox2::legacy::TruncateToCapacity, argv[i + 1] };
            } else {
