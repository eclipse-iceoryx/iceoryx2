...
This directory contains a standalone Bazel example demonstrating how to use iceoryx2 with Bazel modules (bzlmod).
...
Terminal 1 (subscriber):
```bash
bazel run //cxx:subscriber
```

Terminal 2 (publisher):
```bash
bazel run //cxx:publisher
```
