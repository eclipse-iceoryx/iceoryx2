...
| cross language communication generated types | [C++](cxx/cross_language_communication_generated_types)                                                                                                                                         | Make a generated C++ type (one you cannot modify to add an `IOX2_TYPE_NAME` member) cross-language compatible by attaching its type name externally with `IOX2_DEFINE_TYPE_NAME`.                               |
...
| event based communication                    | [C++](cxx/event_based_communication) [Python](python/event_based_communication) [Rust](rust/event_based_communication)                                                                          | Define multiple events like publisher/subscriber created or removed, send sample, received sample, deliver history etc. and react on them for a fully event driven communication setup.                         |
...
| flatbuffer publish subscribe                 | [C++](cxx/flatbuffer_publish_subscribe) [Python](python/flatbuffer_publish_subscribe) [Rust](rust/flatbuffer_publish_subscribe)                                                                 | Native flatbuffer integration and sending fully dynamic unbounded data types between a publisher and a subscriber.                                                                                              |
| flatbuffer request response                  | [C++](cxx/flatbuffer_request_response) [Rust](python/flatbuffer_request_response) [Rust](rust/flatbuffer_request_response)                                                                      | Native flatbuffer integration and sending fully dynamic unbounded data types between a client and a server.                                                                                                     |
...
| publish subscribe with backpressure          | [C](c/publish_subscribe_with_backpressure) [C++](cxx/publish_subscribe_with_backpressure) [Rust](rust/publish_subscribe_with_backpressure)                                                      | Communication between multiple processes with a [publish subscribe messaging pattern](https://en.wikipedia.org/wiki/Publish–subscribe_pattern) and custom backpressure handling.                                |
...
| request response with events                 | [Rust](rust/request_response_with_events)                                                                                                                                                       | Sending requests from one or many clients to one or many servers and receive a stream of responses with notifications for single `Listener` in a event service.                                                 |
...
> [!IMPORTANT]
> Each integration has its own workspace, environment and setup e.g. a
> sourced ROS 2 installation. In the folder of every integration example you
> can find the instructions to build and run it

| Name          | Integration                                                  | Description                                           |
| ------------- | ------------------------------------------------------------ | ----------------------------------------------------- |
| ros2 examples | [ROS 2](../integrations/ros2/examples)                       | Shows how to communicate with ROS 2 via a gateway     |
| ros2 package  | [ROS 2](../integrations/ros2/colcon/examples/src/demo_nodes) | Shows how to build iceoryx2 applications with colcon. |
