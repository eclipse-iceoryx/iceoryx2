...
pub mod service_event {
    ...
    pub fn settings_can_be_modified_via_custom_config<Sut: Service>() {
        let test = Test::<Sut>::new();
        let service_name = generate_service_name();
        let mut custom_config = test.config().clone();
    ...
    pub fn notifier_emits_nothing_when_no_events_are_configured<Sut: Service>() {
        ...
        let received_events = listener.try_wait(|_| {}).unwrap();
    ...
    pub fn notifier_emits_create_and_dropped_event_id<Sut: Service>() {
        ...
        listener
            .try_wait(|event| {
                received_events += event.count;
                assert_that!(event.id, eq notifier_created);
            })
            .unwrap();
        assert_that!(received_events, eq 1);
        ...
        listener
            .try_wait(|event| {
                received_events += event.count;
                assert_that!(event.id, eq notifier_dropped);
            })
            .unwrap();
        assert_that!(received_events, eq 1);
    }
            ...
            for (i, notifier) in notifiers.iter().enumerate() {
                ...
                for listener in &mut listeners {
                    ...
                    listener
                        .try_wait(|event| {
                            received_events += event.count;
                            assert_that!(event.id, eq EventId::new(i + 3));
                        })
                        .unwrap();
                    assert_that!(received_events, eq 1);
                }
        ...
        for _ in 0..NUMBER_OF_ITERATIONS {
            ...
            for listener in &mut listeners {
                ...
                listener
                    .try_wait(|event| {
                        received_events += event.count;
                        assert_that!(received_event_ids[event.id.as_value()], eq false);
                        received_event_ids[event.id.as_value()] = true;
                    })
                    .unwrap();
                assert_that!(received_events, eq MAX_NOTIFIERS as u64);
            }
        }
    }
    ...
    pub fn number_of_nodes_works<Sut: Service>() {
        let test = Test::<Sut>::new();
        let main_node = test.create_node();
        let service_name = generate_service_name();
    ...
    pub fn try_wait_does_not_block<Sut: Service>() {
        ...
        assert_that!(listener.try_wait(|_| {}), eq Ok(0));
    }
    ...
    pub fn timed_wait_blocks_for_at_least_timeout<Sut: Service>() {
        ...
        assert_that!(listener.timed_wait(|_| {}, TIMEOUT), eq Ok(0));
    ...
    pub fn timed_wait_blocks_until_notification<Sut: Service>() {
        wait_blocks_until_notification(|l: &Listener<Sut>| {
            let number_of_notifications = l
                .timed_wait(
                    |event| {
                        assert_that!(event.id, eq EventId::new(13));
                    },
                    TIMEOUT * 1000,
                )
                .unwrap();
    ...
    pub fn deadline_can_be_set<S: Service>() {
        ...
        let service_name = generate_service_name();
        test.config_mut().defaults.event.deadline = None;
        let node = test.create_node();
    ...
    pub fn deadline_can_be_disabled<S: Service>() {
        const DEADLINE: Duration = Duration::from_secs(556);
        let mut test = Test::<S>::new();
        test.config_mut().defaults.event.deadline = Some(DEADLINE);
        let service_name = generate_service_name();
        let node = test.create_node();
    ...
    pub fn notifier_is_informed_when_deadline_was_missed<S: Service>() {
        ...
        assert_that!(listener.try_wait(|_| {}).unwrap(), eq 1);
    ...
    pub fn notifier_does_not_notify_listener_from_same_node_id_when_requested<Sut: Service>() {
        let test = Test::<Sut>::new();
        let service_name = generate_service_name();
        let node_1 = test.create_node();
        let node_2 = test.create_node();
        let event_id = EventId::new(23);
