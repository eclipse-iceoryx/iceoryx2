...
//!  * `logger_console` - Include the [console logger](`iceoryx2-loggers::console::Logger`) and use it as the default log backend
//!  * `logger_buffer` - Include the [buffer logger](`iceoryx2-loggers::buffer::Logger`), which can be set at runtime
//!  * `logger_file` - Include the [file logger](`iceoryx2-loggers::file::Logger`), which can be set at runtime
