...
mod service_publish_subscribe {
    ...
    #[test]
    fn sample_origin_is_tracked_correctly<Sut: Service>() {
        let service_name = generate_name();
        let sut = Sut::new(&service_name)
            .publish_subscribe()
            .create::<u64>()
    ...
    #[test]
    fn sample_from_dropped_publisher_does_not_block_new_publishers<Sut: Service>() {
        let service_name = generate_name();
        let sut = Sut::new(&service_name)
            .publish_subscribe()
            .max_publishers(1)
        ...
        let publisher = sut.publisher().create().unwrap();
        assert_that!(publisher.send_copy(PAYLOAD), eq Ok(1));
        //TODO: why does it deadlock here?
