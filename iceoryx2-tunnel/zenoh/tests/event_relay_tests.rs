...
mod event_relay_tests {
    ...
    fn announces_relays_on_zenoh<S: Service>() {
        ...
        let service = node
            ...
            .open_or_create()
            .unwrap();
        let static_config = S::details(&service_name, &iceoryx_config, MessagingPattern::Event)
            .unwrap()
            .unwrap()
            .static_details;
        ...
        // Create a relay
        let _relay = backend.relay_builder().event(&static_config).create();
        ...
        match zenoh_reply.recv_timeout(Duration::from_millis(100)) {
            Ok(Some(reply)) => match reply.result() {
                Ok(sample) => {
                    ...
                    assert_that!(received_static_config.event(), eq service.static_config());
                }
