    ...
    for UnixDatagramWaiter<E, Storage>
{
    ...
    fn create(
        ...
        let dir_msg = format!("Unable to create target directory \"{}\"", config.path_hint);
        if !fail!(from origin, when Directory::does_exist(&config.path_hint),
            with ListenerCreateError::Creation,
               "{} since the system is unable to determine if the directory even exists.", dir_msg)
        {
            match Directory::create(&config.path_hint, DIR_PERMISSIONS) {
                ...
                Err(e) => {
                    fail!(from origin, with ListenerCreateError::Creation,
                        "{} due to a failure while creating the service root directory ({:?}).", dir_msg, e);
                }
            }
            trace!(
                "Created service root directory \"{}\" since it did not exist before.",
                config.path_hint
            );
        }
