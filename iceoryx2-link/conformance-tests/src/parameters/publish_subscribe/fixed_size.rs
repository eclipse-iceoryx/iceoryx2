...
use crate::parameters::{FixedSizePayload, PublishSubscribePayload};

impl<T> PublishSubscribePayload for FixedSizePayload<T>
where
    T: IceoryxSend + ZeroCopySend + TypeName + Debug + Copy + PartialEq + From<u64> + 'static,
{
    ...
    fn send<S: Service, H: ZeroCopySend + Debug + Default>(
        ...
        let mut sample = publisher.loan_uninit().expect("sample is loaned");
        *sample.user_header_mut() = header;
        sample.write_payload(value).send().expect("sample is sent");
    }
    ...
    fn receive<S: Service, H: ZeroCopySend + Debug + Copy>(
        ...
        subscriber
            ...
            .map(|sample| (*sample.user_header(), *sample.payload()))
    }
}
