...
pub trait Req: AtomicInteger + Debug + BitOr + BitAnd {
...
pub fn atomic_compare_exchange_weak_failure_works<T: Req>() {
    ...
    let result = sut.compare_exchange_weak(n_outdated, n_new, Ordering::Relaxed, Ordering::Relaxed);
...
pub fn atomic_compatibility_fetch_update_success_works<T: Req>() {
    ...
    let result_sut = sut.fetch_update(Ordering::Relaxed, Ordering::Relaxed, ok_fetch_update::<T>);
    let result_compat =
...
pub fn atomic_compatibility_fetch_update_failure_works<T: Req>() {
    ...
    let result_sut = sut.fetch_update(Ordering::Relaxed, Ordering::Relaxed, err_fetch_update::<T>);
...
pub fn atomic_placement_default_works<T: Req>() {
    ...
    assert_that!(unsafe { (*raw_memory).load(Ordering::Relaxed) }, eq T::default());
}
