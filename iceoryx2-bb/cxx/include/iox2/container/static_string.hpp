...
class StaticString {
  ...
  public:
    ...
    template <typename T>
    auto find_first_of(T const& str, SizeType pos = 0U) const -> IsStaticStringOrCharArray<T, Optional<SizeType>> {
        if (pos > m_size) {
            return nullopt;
        }
    ...
    template <typename T>
    auto find_last_of(T const& str, SizeType pos = N) const -> IsStaticStringOrCharArray<T, Optional<SizeType>> {
        if (m_size == 0) {
            return nullopt;
        }
