...
impl<T: Leakable, Ptr: GenericPointer> Leakable for MetaSlotMap<T, Ptr> {
    unsafe fn leak_in_place(this: *mut Self) {
        let this = unsafe { &mut *this };
        for element in this.data.iter_mut().flatten() {
            unsafe { T::leak_in_place(element) };
        }
    }
}
...
impl<T, Ptr: GenericPointer> MetaSlotMap<T, Ptr> {
    ...
    pub(crate) unsafe fn store_value(&mut self, key: SlotMapKey, value: T) -> bool {
        ...
        if data_idx != INVALID {
            ...
            let n = unsafe { self.data_next_free_index.pop_impl() }.expect(
    ...
    pub(crate) unsafe fn remove_impl(&mut self, key: SlotMapKey) -> Option<T> {
        ...
        if data_idx != INVALID {
            ...
            let push_result = unsafe { self.data_next_free_index.push_impl(data_idx) };
            debug_assert!(push_result);
            unsafe { self.release_free_index(key.0) };
            self.idx_to_data[key.0] = INVALID;
            self.len -= 1;
            ret
        } else {
...
impl<T> RelocatableContainer for RelocatableSlotMap<T> {
    unsafe fn new_uninit(capacity: usize) -> Self {
        Self {
            ...
            idx_to_data: unsafe { RelocatableVec::new_uninit(capacity) },
            idx_to_data_free_list: unsafe { RelocatableVec::new_uninit(capacity) },
            data: unsafe { RelocatableVec::new_uninit(capacity) },
            data_next_free_index: unsafe { RelocatableQueue::new_uninit(capacity) },
