...
use crate::{
    api::AssertNonNullHandle, api::HandleToType, iox2_unique_client_id_h, iox2_unique_client_id_t,
};
...
pub struct iox2_request_header_storage_t {
    internal: [u8; 72], // core::mem::size_of::<Option<RequestHeader>>()
}

#[repr(C)]
#[iceoryx2_ffi(RequestHeader)]
pub struct iox2_request_header_t {
    pub value: iox2_request_header_storage_t,
    deleter: fn(*mut iox2_request_header_t),
}

impl iox2_request_header_t {
    pub(super) fn init(&mut self, header: RequestHeader, deleter: fn(*mut iox2_request_header_t)) {
...
/// The immutable pointer to the underlying `RequestHeader`
pub type iox2_request_header_ptr = *const RequestHeader;
/// The mutable pointer to the underlying `RequestHeader`
pub type iox2_request_header_ptr_mut = *mut RequestHeader;

impl AssertNonNullHandle for iox2_request_header_h {
    fn assert_non_null(self) {
        debug_assert!(!self.is_null());
    }
}

impl AssertNonNullHandle for iox2_request_header_h_ref {
    fn assert_non_null(self) {
        debug_assert!(!self.is_null());
        unsafe {
            debug_assert!(!(*self).is_null());
...
impl HandleToType for iox2_request_header_h {
    type Target = *mut iox2_request_header_t;
...
impl HandleToType for iox2_request_header_h_ref {
    type Target = *mut iox2_request_header_t;
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn iox2_request_header_client_id(
...
/// * `header_handle` is valid and non-null
#[unsafe(no_mangle)]
pub unsafe extern "C" fn iox2_request_header_payload_offset(
    header_handle: iox2_request_header_h_ref,
) -> u64 {
    header_handle.assert_non_null();
    unsafe {
