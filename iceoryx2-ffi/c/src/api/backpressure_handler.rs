...
use iceoryx2::port::{BackpressureAction, BackpressureInfo};
...
pub enum iox2_backpressure_action_e {
    /// Use an action which is derived from the `BackpressureStrategy`
    FOLLOW_BACKPRESSUREY_STRATEGY,
...
impl From<iox2_backpressure_action_e> for BackpressureAction {
    fn from(value: iox2_backpressure_action_e) -> Self {
        match value {
            iox2_backpressure_action_e::FOLLOW_BACKPRESSUREY_STRATEGY => {
                BackpressureAction::FollowBackpressureyStrategy
            }
            iox2_backpressure_action_e::RETRY => BackpressureAction::Retry,
            iox2_backpressure_action_e::DISCARD_DATA => BackpressureAction::DiscardData,
            iox2_backpressure_action_e::DISCARD_DATA_AND_FAIL => {
...
pub struct iox2_backpressure_info_h;
pub type iox2_backpressure_info_h_ref = *const iox2_backpressure_info_h;

pub(crate) fn backpressure_info_cast(info: &BackpressureInfo) -> iox2_backpressure_info_h_ref {
    info as *const _ as iox2_backpressure_info_h_ref
}

fn backpressure_info_as_type<'a>(info: iox2_backpressure_info_h_ref) -> &'a BackpressureInfo {
    debug_assert!(!info.is_null());
    unsafe { &*(info as *const _ as *const BackpressureInfo) }
}
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn iox2_backpressure_info_service_id(
    info_handle: iox2_backpressure_info_h_ref,
    service_id: *mut iox2_buffer_16_align_4_t,
) {
    unsafe {
        iox2_buffer_16_align_4_t::write(
            service_id,
            backpressure_info_as_type(info_handle)
                .service_id
                .to_be_bytes(),
        );
    }
...
/// * `service_id` must not be a null pointer
#[unsafe(no_mangle)]
pub unsafe extern "C" fn iox2_backpressure_info_receiver_port_id(
    info_handle: iox2_backpressure_info_h_ref,
...
/// * `service_id` must not be a null pointer
#[unsafe(no_mangle)]
pub unsafe extern "C" fn iox2_backpressure_info_sender_port_id(
    info_handle: iox2_backpressure_info_h_ref,
...
/// * `info_handle` must be a valid handle
#[unsafe(no_mangle)]
pub unsafe extern "C" fn iox2_backpressure_info_retries(
    info_handle: iox2_backpressure_info_h_ref,
) -> u64 {
    backpressure_info_as_type(info_handle).retries
}
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn iox2_backpressure_info_elapsed_time(
    info_handle: iox2_backpressure_info_h_ref,
    seconds: *mut u64,
    nanoseconds: *mut u32,
) {
    ...
    let elapsed = backpressure_info_as_type(info_handle).elapsed_time;
...
pub type iox2_backpressure_handler = extern "C" fn(
    iox2_backpressure_info_h_ref,
