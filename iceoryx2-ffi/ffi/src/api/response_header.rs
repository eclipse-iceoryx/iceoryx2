...
use crate::{
    api::AssertNonNullHandle, api::HandleToType, iox2_unique_server_id_h, iox2_unique_server_id_t,
};
...
pub struct iox2_response_header_storage_t {
    internal: [u8; 48], // core::mem::size_of::<Option<ResponseHeader>>()
}

#[repr(C)]
#[iceoryx2_ffi(ResponseHeader)]
pub struct iox2_response_header_t {
    pub value: iox2_response_header_storage_t,
    deleter: fn(*mut iox2_response_header_t),
}

impl iox2_response_header_t {
    pub(super) fn init(
        &mut self,
...
/// The immutable pointer to the underlying `ResponseHeader`
pub type iox2_response_header_ptr = *const ResponseHeader;
/// The mutable pointer to the underlying `ResponseHeader`
pub type iox2_response_header_ptr_mut = *mut ResponseHeader;

impl AssertNonNullHandle for iox2_response_header_h {
    fn assert_non_null(self) {
        debug_assert!(!self.is_null());
    }
}

impl AssertNonNullHandle for iox2_response_header_h_ref {
    fn assert_non_null(self) {
        debug_assert!(!self.is_null());
        unsafe {
            debug_assert!(!(*self).is_null());
...
impl HandleToType for iox2_response_header_h {
    type Target = *mut iox2_response_header_t;
...
impl HandleToType for iox2_response_header_h_ref {
    type Target = *mut iox2_response_header_t;
...
pub unsafe extern "C" fn iox2_response_header_server_id(
    ...
    let id = header.value.as_ref().server_id();
