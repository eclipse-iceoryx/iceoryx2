...
// 4. FAQ
TYPED_TEST(ServicePublishSubscribeTest, blub) {
...

    {
        ...
        iox::UninitializedArray<DummyData, SLICE_MAX_LENGTH + 1, iox::ZeroedBuffer> elements;
        for (auto& item : elements) {
            new (&item) DummyData {};
        }
        ...
        auto recv_result = sut_subscriber.receive();
        ASSERT_FALSE(recv_result.has_error());

        // publisher has to reallocate the data segment
        payload = iox::ImmutableSlice<DummyData>(elements.begin(), SLICE_MAX_LENGTH + 1);
        sample_uninit = sut_publisher.loan_slice_uninit(SLICE_MAX_LENGTH + 1).expect("");
        send_sample = sample_uninit.write_from_slice(payload);
        send(std::move(send_sample)).expect("");
    ...
    auto recv_result = sut_subscriber.receive();
    ASSERT_TRUE(recv_result.has_error());
    EXPECT_THAT(recv_result.error(), Eq(ReceiveError::UnableToMapSendersDataSegment));
}
} // namespace
