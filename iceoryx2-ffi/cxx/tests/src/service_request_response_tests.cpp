...
// TODO:
// 1. rename test
// 2. remove set_log_level
// 4. FAQ
// 5. check why it only fails when the server goes out of scope
TYPED_TEST(ServiceRequestResponseTest, blub) {
    set_log_level_from_env_or(LogLevel::Trace);
    constexpr ServiceType SERVICE_TYPE = TestFixture::TYPE;
    constexpr auto SLICE_MAX_LENGTH = 1;
    ...
    auto sut_client = service.client_builder()
                          .initial_max_slice_len(SLICE_MAX_LENGTH)
                          .allocation_strategy(AllocationStrategy::BestFit)
                          .create()
                          .expect("");
    auto request = sut_client.loan_slice_uninit(SLICE_MAX_LENGTH).expect("loan successful");
    auto initialized_request = request.write_from_fn([&](auto byte_idx) { return byte_idx; });
...

    {
        ...
        auto service = node.service_builder(service_name)
                           ...
                           .open_or_create()
                           .expect("");
        auto sut_server = service.server_builder()
                              .initial_max_slice_len(SLICE_MAX_LENGTH)
                              .allocation_strategy(AllocationStrategy::BestFit)
                              .create()
                              .expect("");
        ...
        auto active_request = sut_server.receive().expect("receive successful");
        ASSERT_TRUE(active_request.has_value());
        auto response = active_request->loan_slice_uninit(SLICE_MAX_LENGTH).expect("loan successful");
        auto initialized_response = response.write_from_fn([&](auto byte_idx) { return byte_idx; });
        send(std::move(initialized_response)).expect("send successful");

        auto received_response = pending_response.receive();
        ASSERT_FALSE(received_response.has_error());

        // server has to reallocate the data segment
        response = active_request->loan_slice_uninit(SLICE_MAX_LENGTH + 100).expect("loan successful"); // NOLINT
        initialized_response = response.write_from_fn([&](auto byte_idx) { return byte_idx; });
        send(std::move(initialized_response)).expect("send successful");
        // server goes out of scope and closes the reallocated data segment as it was not yet mapped by the client
    }

    auto response = pending_response.receive();
    ASSERT_TRUE(response.has_error());
    EXPECT_THAT(response.error(), Eq(ReceiveError::UnableToMapSendersDataSegment));
}
TYPED_TEST(ServiceRequestResponseTest, blub2) {
    set_log_level_from_env_or(LogLevel::Trace);
    constexpr ServiceType SERVICE_TYPE = TestFixture::TYPE;
    constexpr auto SLICE_MAX_LENGTH = 1;
    ...
    ASSERT_FALSE(active_request.has_error());
    {
        ...
        // send and receive once so that the server maps the data segment
        auto request = sut_client.loan_slice_uninit(SLICE_MAX_LENGTH).expect("loan successful");
        auto initialized_request = request.write_from_fn([&](auto byte_idx) { return byte_idx; });
        auto pending_response = send(std::move(initialized_request)).expect("send successful");

        active_request = sut_server.receive();
        ASSERT_FALSE(active_request.has_error());

        // client has to reallocate the data segment
        request = sut_client.loan_slice_uninit(SLICE_MAX_LENGTH + 100).expect("loan successful");
        initialized_request = request.write_from_fn([&](auto byte_idx) { return byte_idx; });
        pending_response = send(std::move(initialized_request)).expect("send successful");
        // client goes out of scope and closes the reallocated data segment as it was not yet mapped by the server
    }

    active_request = sut_server.receive();
    ASSERT_TRUE(active_request.has_error());
    EXPECT_THAT(active_request.error(), Eq(ReceiveError::UnableToMapSendersDataSegment));
}
} // namespace
