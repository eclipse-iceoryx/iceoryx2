...
from flatbuffer_types.Helper import (create_bounded_data,
                                     create_unbounded_data,
                                     flatbuffer_tests_schema_path,
                                     get_schema_file)
...
schema_bounded = "bounded_data.bfbs"
...
def test_open_fails_when_request_schema_is_not_the_same(
    ...
    request_schema_file_path = get_schema_file(schema_bounded)
    response_schema_file_path = get_schema_file(schema_unbounded)
    incompatible_schema_file_path = get_schema_file(schema_incompatible)
    config = iox2.testing.generate_isolated_config()
    node = iox2.NodeBuilder.new().config(config).create(service_type)
    service_name = iox2.testing.generate_service_name()
...
def test_schema_path_lookup_works_when_creating_a_service(
    ...
    config = iox2.testing.generate_isolated_config()
    config.global_cfg.service.flatbuffer_schema_path = flatbuffer_tests_schema_path()
