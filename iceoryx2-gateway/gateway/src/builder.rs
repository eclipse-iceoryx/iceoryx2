...
//! Type-state builder for the [`Gateway`].
//!
//! [`GatewayBuilder`] selects between polled and reactive operation. The
...
use iceoryx2_gateway_backend::traits::{Backend, BackendBuilder, ReactiveBackendBuilder};
...
use crate::gateway::{Config, CreationError, Gateway};

/// Marker for a [`GatewayBuilder`] whose mode has not yet been chosen.
#[derive(Debug)]
pub struct Unconfigured;

/// Marker for a [`GatewayBuilder`] in polled mode.
#[derive(Debug)]
pub struct Polled;

/// Marker for a [`GatewayBuilder`] in reactive mode.
#[derive(Debug)]
pub struct Reactive;

/// Builder for [`Gateway`].
///
/// Constructed via [`GatewayBuilder::new`] in the [`Unconfigured`] state. The
/// builder must transition through [`GatewayBuilder::polled`] or
/// [`GatewayBuilder::reactive`] before calling `create()`.
///
    ...
    B: Backend<S> + Debug,
{
    ...
    /// Sets the [`iceoryx2::config::Config`] used for the gateway's iceoryx
    /// services.
    ...
    B: Backend<S> + Debug,
{
    ...
    /// Selects polled mode. The resulting gateway must be driven by repeated
    /// calls to [`Gateway::discover`] and [`Gateway::propagate`].
    pub fn polled(self) -> GatewayBuilder<S, B, Polled> {
        GatewayBuilder {
    ...
    for<'b> B::Builder<'b>: ReactiveBackendBuilder<S>,
{
    ...
    pub fn reactive(self) -> GatewayBuilder<S, B, Reactive> {
        GatewayBuilder {
            gateway_config: self.gateway_config,
            iceoryx_config: self.iceoryx_config,
            backend_config: self.backend_config,
    ...
    B: Backend<S> + Debug,
{
    ...
    pub fn create(self) -> Result<Gateway<S, B>, CreationError> {
        create_polled::<S, B>(
            self.gateway_config.unwrap_or_default(),
    ...
    for<'b> B::Builder<'b>: ReactiveBackendBuilder<S>,
{
    ...
    pub fn create<W>(self) -> Result<(Gateway<S, B>, Listener<W>), CreationError>
    where
        W: Service,
...
/// Builds a gateway in polled mode.
    ...
    B: Backend<S> + Debug,
{
    ...
    create(gateway_config, iceoryx_config, backend)
}

/// Builds a gateway in reactive mode.
    ...
    for<'b> B::Builder<'b>: ReactiveBackendBuilder<S, WakeService = W>,
{
    ...
    let service_name_str = format!("iox2://gateway/event/{}", reactive_node.id());
    ...
    let gateway = create(gateway_config, iceoryx_config, backend)?;

    Ok((gateway, listener))
}

/// Finalizes the build of the Gateway.
fn create<S, B>(
    gateway_config: Config,
    iceoryx_config: iceoryx2::config::Config,
    backend: B,
    ...
    B: Backend<S> + Debug,
{
    ...
    trace!(
        from origin,
        "Building Gateway:\n{:?}\n{:?}",
        &gateway_config, &iceoryx_config
    );

    let gateway_node = fail!(
        from origin,
        when NodeBuilder::new().config(&iceoryx_config).create::<S>(),
        with CreationError::Node,
        "Failed to create Gateway node"
    );

    let local_discovery = match &gateway_config.discovery_service {
        Some(service_name) => {
            ...
            let subscriber = fail!(
                from origin,
                when DiscoverySubscriber::create(&gateway_node, service_name),
