...
pub mod publish_subscribe_propagation {
    ...
    fn propagate_struct_payloads<S: Service, B: Backend<S> + Debug, T: Testing>(num: usize) {
        ...
        // Wait for gateway on host B to discover the service on host A
        T::retry(
            || {
                gateway_b.discover_over_backend().unwrap();
                let service_discovered = gateway_b.bridged_services().len() == 1;
        ...
        // Create a subscriber to connect to the bridged service
        ...
        for i in 0..num {
            ...
            *sample_sent_at_a.user_header_mut() = user_header_sent_at_a.clone();
            ...
            gateway_a.propagate().unwrap();
            gateway_b.propagate().unwrap();
    ...
    fn propagate_slice_payloads<S: Service, B: Backend<S> + Debug, T: Testing>(num: usize) {
        ...
        // Create a subscribe to connect to the bridged service
        let node_b = NodeBuilder::new()
            .config(&iceoryx_config_b)
            .create::<S>()
            .unwrap();
    ...
    >() {
        ...
        gateway_a.discover_over_iceoryx().unwrap();
        assert_that!(gateway_a.bridged_services().len(), eq 2);
        assert_that!(gateway_a.bridged_services().contains(service_a1.service_hash()), eq true);
        assert_that!(gateway_a.bridged_services().contains(service_a2.service_hash()), eq true);
        ...
        // Wait for gateway on host B to discover both services on host A
        T::retry(
            || {
                gateway_b.discover_over_backend().unwrap();
                if gateway_b.bridged_services().len() == 2 {
                    return Ok(());
                }
    ...
    pub fn propagated_payloads_do_not_loop_back<S: Service, B: Backend<S> + Debug, T: Testing>() {
        ...
        assert_that!(gateway.bridged_services().contains(service.service_hash()), eq true);
