...
pub mod event_discovery {
    ...
    >() {
        ...
        assert_that!(gateway.bridged_services().contains(&service_hash), eq true);
        ...
        assert_that!(gateway.bridged_services().contains(&service_hash), eq false);
    }
    ...
    >() {
        ...
        assert_that!(gateway_b.bridged_services().len(), eq 1);
        ...
        T::retry(
            || {
                ...
                if gateway_a.bridged_services().len() == 1 {
                    return Ok(());
                }
        ...
        assert_that!(gateway_a.bridged_services().len(), eq 0);
        assert_that!(gateway_a.bridged_services().contains(&service_hash), eq false);
    }
    ...
    >() {
        ...
        gateway_b.discover_over_iceoryx().unwrap();
        gateway_c.discover_over_iceoryx().unwrap();
        ...
        drop(service_c);
        gateway_c.discover_over_iceoryx().unwrap();
        assert_that!(gateway_c.bridged_services().len(), eq 0);
    ...
    pub fn detects_ungraceful_remote_departure<S: Service, B: Backend<S> + Debug, T: Testing>() {
        ...
        T::retry(
            || {
                // Service from crashed gateway should be detected as removed.
                gateway_a.discover_over_backend().unwrap();
