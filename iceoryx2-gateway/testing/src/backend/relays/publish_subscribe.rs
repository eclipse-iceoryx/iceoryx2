...
use iceoryx2_gateway_backend::traits::{PublishSubscribeRelay, RelayBuilder};
...
impl<S: Service> PublishSubscribeRelay<S> for Relay<S> {
    ...
    fn send(
        &self,
        sample: iceoryx2_gateway_backend::types::publish_subscribe::Sample<S>,
    ...
    fn receive<LoanError>(
        &self,
        loan: &mut iceoryx2_gateway_backend::types::publish_subscribe::LoanFn<'_, S, LoanError>,
    ) -> Result<
        Option<iceoryx2_gateway_backend::types::publish_subscribe::SampleMut<S>>,
        Self::ReceiveError,
    > {
