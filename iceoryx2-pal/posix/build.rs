...
fn main() {
    ...
    configure_link_libraries(target_os.as_str());
}
...
#[cfg(not(feature = "libc_platform"))]
fn configure_link_libraries(target_os: &str) {
    match target_os {
        "freebsd" => {
            freebsd::configure_link_libraries();
        }
        "linux" => {
            linux::configure_link_libraries();
        }
        "macos" => {
            macos::configure_link_libraries();
        }
        "qnx" => {
            qnx::configure_link_libraries();
        }
        "windows" => {
            windows::configure_link_libraries();
        }
        _ => panic!("Unsupported target OS: {}", target_os),
    }
}
