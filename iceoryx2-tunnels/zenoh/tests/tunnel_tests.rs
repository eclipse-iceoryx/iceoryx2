...
mod zenoh_tunnel {
    ...
    fn discovers_remote_services_via_zenoh<S: Service>() {
        ...
        const TIME_BETWEEN_DISCOVERY_ATTEMPTS: Duration = Duration::from_millis(100);
        const NUM_DISCOVERY_ATTEMPTS: usize = 25;
        retry(
            || {
                tunnel_a.discover().unwrap();
            ...
            TIME_BETWEEN_DISCOVERY_ATTEMPTS,
            Some(NUM_DISCOVERY_ATTEMPTS),
        );
    }
    ...
    fn propagates_n_struct_payloads<S: Service>(sample_count: usize) {
        ...
        for i in 0..sample_count {
            ...
            // Receive
            const TIME_BETWEEN_ATTEMPTS: Duration = Duration::from_millis(100);
            const NUM_RETRIES: usize = 25;
            retry(
                || {
