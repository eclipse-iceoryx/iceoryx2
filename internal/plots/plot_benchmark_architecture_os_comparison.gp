...
plot 'benchmark_architecture_os_comparison.dat' using (0.5*$3):0:(0.5*$3):((myBoxWidth)/2.):ytic(sprintf("%s\n{/*0.8 %s}", stringcolumn(1), stringcolumn(2))) with boxxy lc rgb color_graph_box_multicore

# plot 'benchmark_architecture_os_comparison.dat' using (0.5*$4):0:(0.5*$4):((myBoxWidth)/2.):ytic(sprintf("%s\n{/*0.8 %s}", stringcolumn(1), stringcolumn(2))) with boxxy lc rgb color_graph_box, \
