...
constexpr Duration TIMEOUT = Duration::from_milliseconds(100);
...
TYPED_TEST(WaitSetTest, attaching_different_elements_works) {
    ...
    for (uint64_t idx = 0; idx < NUMBER_OF_INTERVALS; ++idx) {
        guards.emplace_back(sut.attach_interval(Duration::from_seconds(idx + 1)).expect(""));
    ...
    for (uint64_t idx = 0; idx < NUMBER_OF_DEADLINES; ++idx) {
        auto listener = this->create_listener();
        guards.emplace_back(sut.attach_deadline(listener, Duration::from_seconds(idx + 1)).expect(""));
        listeners.emplace_back(std::move(listener));
...
TYPED_TEST(WaitSetTest, attaching_same_deadline_twice_fails) {
    ...
    auto result_1 = sut.attach_deadline(listener, Duration::from_seconds(1));
    auto result_2 = sut.attach_deadline(listener, Duration::from_seconds(1));
...
TYPED_TEST(WaitSetTest, interval_attachment_blocks_for_at_least_timeout) {
    ...
    ASSERT_THAT(elapsed, Ge(static_cast<decltype(elapsed)>(TIMEOUT.to_milliseconds())));
}
...
TYPED_TEST(WaitSetTest, deadline_attachment_wakes_up_when_notified) {
    ...
    std::thread notifier_thread([&]() -> auto {
        std::this_thread::sleep_for(std::chrono::milliseconds(TIMEOUT.to_milliseconds()));
...
TYPED_TEST(WaitSetTest, triggering_everything_works) {
    ...
    for (uint64_t idx = 0; idx < NUMBER_OF_INTERVALS; ++idx) {
        guards.emplace_back(sut.attach_interval(Duration::from_nanoseconds(1)).expect(""));
    }
