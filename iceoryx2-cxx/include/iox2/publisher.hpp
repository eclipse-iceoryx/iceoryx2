...
inline auto Publisher<S, Payload, UserHeader>::loan_flatbuffer()
    ...
    iox2_sample_mut_create_resizable_memory(&sample.m_sample.m_handle, nullptr, &resizable_memory_handle);
...
template <ServiceType S, typename Payload, typename UserHeader>
template <typename T, typename>
inline auto Publisher<S, Payload, UserHeader>::loan() -> bb::Expected<SampleMut<S, Payload, UserHeader>, LoanError> {
    auto sample = loan_uninit();

    if (!sample.has_value()) {
        return bb::err(sample.error());
    }
...
inline auto Publisher<S, Payload, UserHeader>::loan_slice_uninit(const uint64_t number_of_elements)
    -> bb::Expected<SampleMutUninit<S, T, UserHeader>, LoanError> {
    SampleMutUninit<S, Payload, UserHeader> sample;
...
template <ServiceType S, typename Payload, typename UserHeader>
inline auto Publisher<S, Payload, UserHeader>::update_connections() -> bb::Expected<void, ConnectionFailure> {
    auto result = iox2_publisher_update_connections(&m_handle);
    if (result != IOX2_OK) {
        return bb::err(iox2::bb::into<ConnectionFailure>(result));
    }
