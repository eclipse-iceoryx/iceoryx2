...
class StaticConfigEvent {
  public:
    ...
    auto notifier_created_event() const -> iox2::legacy::optional<EventId>;

    /// Returns the emitted [`EventId`] when a notifier is dropped.
    auto notifier_dropped_event() const -> iox2::legacy::optional<EventId>;

    /// Returns the emitted [`EventId`] when a notifier is identified as dead.
    auto notifier_dead_event() const -> iox2::legacy::optional<EventId>;
