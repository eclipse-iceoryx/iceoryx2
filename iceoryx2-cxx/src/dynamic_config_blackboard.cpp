...
void DynamicConfigBlackboard::list_readers(
    const iox2::bb::Function<CallbackProgression(ReaderDetailsView)>& callback) const {
...
void DynamicConfigBlackboard::list_writers(
    const iox2::bb::Function<CallbackProgression(WriterDetailsView)>& callback) const {
    auto ctx = internal::ctx(callback);
