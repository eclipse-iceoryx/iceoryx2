...
void DynamicConfigRequestResponse::list_servers(
    const iox2::bb::function<CallbackProgression(ServerDetailsView)>& callback) const {
...
void DynamicConfigRequestResponse::list_clients(
    const iox2::bb::function<CallbackProgression(ClientDetailsView)>& callback) const {
    auto ctx = internal::ctx(callback);
