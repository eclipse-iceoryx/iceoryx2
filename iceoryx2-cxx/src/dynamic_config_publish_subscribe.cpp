...
void DynamicConfigPublishSubscribe::list_publishers(
    const iox2::bb::Function<CallbackProgression(PublisherDetailsView)>& callback) const {
...
void DynamicConfigPublishSubscribe::list_subscribers(
    const iox2::bb::Function<CallbackProgression(SubscriberDetailsView)>& callback) const {
    auto ctx = internal::ctx(callback);
