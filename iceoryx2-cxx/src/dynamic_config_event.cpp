...
void DynamicConfigEvent::list_notifiers(
    const iox2::bb::Function<CallbackProgression(NotifierDetailsView)>& callback) const {
...
void DynamicConfigEvent::list_listeners(
    const iox2::bb::Function<CallbackProgression(ListenerDetailsView)>& callback) const {
    auto ctx = internal::ctx(callback);
