...
fn maps_prefixed_publish_subscribe_services() {
    let sut = PrefixMapping::new(AllowList::new(&["/chatter"]));
    let description =
...
fn maps_service_description_to_topic_description() {
    let sut = PrefixMapping::new(AllowList::new(&["/Camera/FrontRight"]));
...
#[test]
fn empty_allow_list_bridges_no_prefixed_service_outbound() {
...
fn allow_list_restricts_both_directions() {
    ...
    let admitted_service =
        service_description_with_default_settings("ros2://topics/chatter", "std_msgs/msg/String");
    let rejected_service =
        service_description_with_default_settings("ros2://topics/other", "std_msgs/msg/String");

    assert_that!(sut.remote(&admitted_service), is_some);
    assert_that!(sut.remote(&rejected_service), is_none);

    let admitted_topic = TopicDescription {
        topic: TopicName::new("/chatter").expect("valid topic name"),
        type_name: TypeName::new("std_msgs/msg/String").expect("valid type name"),
        qos: QosProfile::default(),
    };
    let rejected_topic = TopicDescription {
        topic: TopicName::new("/other").expect("valid topic name"),
        type_name: TypeName::new("std_msgs/msg/String").expect("valid type name"),
        qos: QosProfile::default(),
    };

    assert_that!(sut.local::<Service>(&admitted_topic), is_some);
    assert_that!(sut.local::<Service>(&rejected_topic), is_none);
}
