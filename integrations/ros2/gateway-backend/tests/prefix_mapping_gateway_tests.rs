...
use iceoryx2_gateway_backend::traits::Passthrough;
...
fn maps_iceoryx_services_onto_ros_topics() {
    let topic = format!(
        "/prefix_mapping_gateway_tests/outbound_{}",
        std::process::id()
    );
...
fn does_not_map_unprefixed_services() {
    ...
    let publish_subscribe_topic =
        format!("/prefix_mapping_gateway_tests/publish_subscribe_topic_{pid}");
    ...
    assert_that!(gateway.bridged_services(), len 1);
}
...
fn does_not_map_event_services() {
    ...
    let publish_subscribe_topic =
        format!("/prefix_mapping_gateway_tests/publish_subscribe_topic_{pid}");
    let event_topic = format!("/prefix_mapping_gateway_tests/event_topic_{pid}");
...
fn maps_ros_topics_onto_iceoryx2_services() {
    let topic = format!(
        "/prefix_mapping_gateway_tests/inbound_{}",
        std::process::id()
    );
